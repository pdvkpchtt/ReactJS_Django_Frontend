...
const PostDropDown = ({
  ...
  const renderItems = () => {
    return items.map((item, key) => (
      ...
      >
        <p
          className={`font-medium select-none text-[14px] px-[12px] transition duration-[250ms] leading-[16px] tracking-[-0.013125em] cursor-pointer ${
            key === 0
              ? "pt-[12px] pb-[6px]"
              : key === items.length - 1
              ? "pt-[6px] pb-[12px]"
  ...
  return (
    <div className={`relative flex flex-col ${styled}`}>
      ...
      >
        ...
        <ArrowIcon style={openState && "transform rotate-180"} />
      </div>
